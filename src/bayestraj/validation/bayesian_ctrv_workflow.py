...
def run_bayesian_ctrv_evaluation(
    *,
    ...
    configured_priors = dataclasses.replace(
        priors,
        turn_rate_prior_abs_rate_deg_s=options.turn_rate_prior_abs_rate_deg_s,
    )
...
def _run_evaluation(
    *,
    ...
    print(
        "Turn-rate prior       : Normal(0, scale); P(|turn rate| > "
...
def _posterior_diagnostics(fit):
    """Return forecast-origin state and process-parameter medians."""
    medians = {
    ...
    return {
        ...
        "posterior_sigma_speed_process_median_mps": medians["sigma_speed_process"],
...
def _print_parameter_summary(predictions):
    ...
    rows = [
        (
            "Median speed at origin",
            f"{windows['posterior_speed_at_origin_median_mps'].median():.3f} m/s",
        ),
        (
