...
def build_prior_curves(priors: bayesian_model.BayesianCTRVPriors):
    ...
    speed_x = np.linspace(
        0.0, _symmetric_quantile(priors.speed_prior_scale), DENSITY_POINT_COUNT
    )
    speed_density = (
        np.sqrt(2.0 / np.pi)
    ...
    turn_x = np.linspace(
        -_symmetric_quantile(turn_scale),
        _symmetric_quantile(turn_scale),
        DENSITY_POINT_COUNT,
    )
    turn_process_mean = float(
        np.rad2deg(1.0 / priors.sigma_turn_rate_process_prior_rate)
    )
    return (
        PriorCurve(
            "prior_initial_speed",
            "Prior: Anfangsgeschwindigkeit",
            r"$v_1$ [m/s]",
            speed_x,
            speed_density,
            0.0,
        ...
        PriorCurve(
            ...
            "Prior: Anfangskurswinkel",
            r"$\theta_1$ [$^\circ$]",
            heading_x,
            np.full_like(heading_x, 1.0 / 360.0),
            -180.0,
            180.0,
            1.0,
            (-180.0, 180.0),
        ...
        PriorCurve(
            ...
            "Prior: initiale Drehrate",
            r"$\omega_1$ [$^\circ$/s]",
            turn_x,
            _normal_density(turn_x, turn_scale),
            -turn_threshold,
            turn_threshold,
            1.0 - priors.turn_rate_prior_tail_probability,
            (-turn_threshold, turn_threshold),
        ),
        _exponential_curve(
            "prior_position_observation_noise",
            "Prior: Positionsmessrauschen",
        ...
        _exponential_curve(
            ...
            priors.sigma_speed_process_prior_upper_mps,
        ...
        _exponential_curve(
            ...
            1.0 / turn_process_mean,
            priors.sigma_turn_rate_process_prior_upper_deg_s,
...
def create_individual_figures(curves, *, show_legend=SHOW_LEGEND):
    ...
    return {
        curve.filename_stem: create_prior_figure(curve, show_legend=show_legend)
        for curve in curves
    }
...
def _draw_prior(axis, curve: PriorCurve, *, show_legend=True) -> None:
    ...
    axis.fill_between(
        ...
        color=CENTRAL_COLOR,
        alpha=0.20,
        label=curve.central_legend_label
        or f"{_format_percentage(curve.central_probability)} innerhalb der Grenze",
    )
    if np.any(~central):
        axis.fill_between(
            curve.x_values,
            ...
            color=TAIL_COLOR,
            alpha=0.22,
            label=f"{_format_percentage(1.0 - curve.central_probability)} außerhalb der Grenze",
        )
    ...
    axis.set_xticks(
        sorted(
            {
                *(tick for tick in base_ticks if x_lower <= tick <= x_upper),
                *curve.thresholds,
            }
        )
    )
...
def _exponential_curve(
    filename_stem, title, x_label, rate, configured_upper, tail_probability
):
    x_values = np.linspace(
        0.0, -np.log(PLOT_TAIL_PROBABILITY) / rate, DENSITY_POINT_COUNT
    )
    return PriorCurve(
        filename_stem,
        title,
