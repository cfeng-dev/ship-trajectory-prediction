...
model {
  ...
  // CTRV transition with inferred process noise per local position axis.
  for (n in 2:N_history) {
    real dt = time_observed[n] - time_observed[n - 1];
    real heading_previous = heading_initial
        + turn_rate * (time_observed[n - 1] - time_observed[1]);
...
generated quantities {
  ...
  real heading_previous = heading_initial
      + turn_rate * (time_observed[N_history] - time_observed[1]);
