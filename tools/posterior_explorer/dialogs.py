...
class SettingsDialog(tk.Toplevel):
    ...
    def __init__(self, parent, panel, group):
        ...
        form = ScrollableForm(self, width=580)
        form.grid(row=1, column=0, sticky="nsew")
        populate_fields(form.body, self.variables)
        form.bind_mouse_wheel()
        actions = tk.Frame(self, bg=CONTROL_BACKGROUND, padx=14, pady=12)
        actions.grid(row=2, column=0, sticky="ew")
        create_styled_button(actions, text="Übernehmen", command=self.apply).pack(
            side="right", padx=(8, 0)
        )
        create_styled_button(actions, text="Abbrechen", command=self.cancel).pack(
            side="right"
        )
